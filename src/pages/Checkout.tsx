...
const Checkout: React.FC = () => {
	...
	const basePrice = 87.4;
	const orderBumpPrice = 48.3;
	const totalPrice = basePrice + (includeOrderBump ? orderBumpPrice : 0);
	...
	const handleGeneratePix = async () => {
		...
		if (!email.length || !phone.length || !name.length || !cpf.length) {
		...
		await fetch("https://api.utmify.com.br/api-credentials/orders", {
			...
			body: JSON.stringify({
				...
				commission: {
					...
					gatewayFeeInCents: totalPrice * 100,
					userCommissionInCents: totalPrice * 100,
				},
				isTest: false,
			}),
		});
					...
					<div className="bg-white rounded-lg shadow-sm p-4">
						...
						<div className="bg-orange-50 border border-orange-100 rounded-lg p-3 mb-4">
							<div className="flex gap-3">
								...
								/>
								<div>
									<h3 className="text-sm font-medium mb-1">100 QUESTÕES GABARITADAS</h3>
						...
						<button
							...
							className="w-full bg-[#1351B4] text-white py-3 rounded-lg text-sm font-medium hover:bg-[#1351B4]/90 transition-colors"
						>
							GERAR PIX
						</button>
					</div>
