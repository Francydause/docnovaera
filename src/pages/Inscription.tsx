...
const validateCPFFromAPI = async (cpf: string): Promise<{ valid: boolean }> => {
...
const Inscription: React.FC = () => {
	...
	const validateCEP = async () => {
		...
		try {
			...
			const mockSchools = [
				{
					id: `escola-${cleanCEP}-1`,
					name: `Escola Municipal ${data.bairro}`,
					type: "Escola pública",
					distance: 1.2,
				},
				{
					id: `escola-${cleanCEP}-2`,
					name: `Colégio Estadual ${data.localidade}`,
					type: "Escola pública",
					distance: 1.8,
				},
				{
					id: `escola-${cleanCEP}-3`,
					name: `EMEF ${data.logradouro}`,
					type: "Escola pública",
					distance: 2.3,
				},
			];
			...
			const addressInfo: AddressInfo = {
				...
				uf: data.uf,
				codigoRegiao: `${data.uf}-${Math.floor(Math.random() * 90 + 10)}-${Math.floor(
					Math.random() * 900 + 100,
				)}`,
				pontoAtendimento: mockSchools[0].name,
				vagasDisponiveis: Math.floor(Math.random() * 50) + 1,
				nearbySchools: mockSchools,
			};

			setAddressInfo(addressInfo);
			setLocationInfo(cleanCEP, mockSchools);
		} catch (err) {
	...
	return (
		<main className="container mx-auto px-4 py-8 flex-grow">
			<div className="max-w-2xl mx-auto">
				<div className="mb-8">
					<h1 className="text-[#1351B4] text-[1.55rem] font-bold mb-2">Verificação de Disponibilidade</h1>
				...
				<div className="bg-white rounded-lg p-6">
					...
					>
						<Search size={24} />
						<span className="text-lg font-semibold">Verificar Disponibilidade</span>
					</button>
						...
						<>
							...
							<div className="mt-8 bg-white rounded-lg p-6">
								...
								<p className="text-gray-600 mb-6">
									Preencha seus dados abaixo para se inscrever no programa Agente Escola do Futuro.
								</p>
					...
					<div className="mt-8 bg-[#FFF9E6] rounded-lg p-6 border-l-4 border-[#FFCD07]">
						...
						<ul className="space-y-4">
							...
							<li className="flex items-start gap-2">
								<span className="text-gray-700">•</span>
								<span>
									A bolsa de capacitação varia entre R$ 3.584,22 a R$ 4.956,13 dependendo da região.
								</span>
							</li>
						</ul>
					</div>
